...
from neuralce.analysis.plot_utils import plot_results
...
def main():
    ...
    # YAML에 n_atoms_orig 명시돼 있으면 supercell 전체 원자 수로 override
    # (exclude_species로 줄어든 graph 원자 수가 아니라 per-atom 정규화용 원본 크기)
    _n_orig_cfg = cfg.get('n_atoms_orig')
    if isinstance(_n_orig_cfg, int):
        if n_atoms_orig != _n_orig_cfg:
            print(f"  n_atoms_orig override from cfg: {n_atoms_orig} → {_n_orig_cfg}")
        n_atoms_orig = _n_orig_cfg
    ...
    # ── Training loop (no patience) — best by (SRCC+R²)/2 ────────────
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if len(val_targets) >= 3:
            ...
            val_score = (rho + r2) / 2.0 if not (np.isnan(rho) or np.isnan(r2)) else -float('inf')
        else:
            val_score = -float('inf')
        ...
        if epoch % 100 == 0:
            print(f"  [epoch {epoch:5d}] train={epoch_loss:.6f} "
                  f"val={val_loss:.6f} score={val_score:.4f} "
                  f"best_score={best_score:.4f}@{best_epoch}")

    print(f"\n  Training done. Best (SRCC+R²)/2={best_score:.4f} "
    ...
    # ── Save ──────────────────────────────────────────────────────────
    cfg_output = cfg.get('retrain', {}).get('output')
    out_path = args.output or cfg_output or f"retrained_{os.path.basename(args.checkpoint)}"
