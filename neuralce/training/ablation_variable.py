...
def load_data(cif_dir, csv_path, spin_pkl_path, id_col, comp_regex,
    ...
    # Spin 우선순위: (1) spin_pkl, (2) CSV 'spins' 컬럼, (3) zeros
    spin_map = {}
...
def build_all_graphs(structures, cutoff, n_shells, max_num_nbr,
                     n_atoms_pad, include_sisj=False, shell_edges=None):
    """Build all graphs, pad to n_atoms_pad, stack.

    Each crystal is padded with zero-rows up to n_atoms_pad atoms, and an
    atom_mask is generated (1 for real atoms, 0 for padded). Uniform datasets
    where every crystal has n_atoms == n_atoms_pad have all-ones masks, so the
    behaviour matches the previous fixed-pool path.
    """
    ...
    for i in range(len(raw['atom_fea'])):
        ...
        if n_at == n_atoms_pad:
            ...
            raise ValueError(
                f"Structure {i} has {n_at} atoms > n_atoms_pad={n_atoms_pad}")

    if n_padded > 0:
        print(f"    Padded {n_padded}/{len(raw['atom_fea'])} structures "
              f"to {n_atoms_pad} atoms")
...
def init_train_state(model, hp, rng_key, dataset, model_name):
    """Initialize model params + optimizer (padded mode, atom_mask 사용)."""
...
def _forward(state, af, nf, nfi, sf, mask, use_spin):
    """Flat forward for a batch (padded mode)."""
    B = af.shape[0]
...
def eval_epoch(state, dataset, indices, use_spin, return_per_comp_srcc=False):
    ...
    for start in range(0, len(indices), BATCH_SIZE):
        ...
        msk = dataset['atom_mask'][idx]
        targets = dataset['energies'][idx]
        B = af.shape[0]

        pred = _forward(state, af, nf, nfi, sf, msk, use_spin)
...
def main():
    global N_ATOMS  # _forward / loss_fn 등이 module-level N_ATOMS를 참조함
    pyrandom.seed(SEED)
    np.random.seed(SEED)
    ...
    # Determine N_ATOMS: uniform이면 그 값, mixed이면 max로 padding
    n_atoms_set = set(s['n_atoms'] for s in structures)
    _n_atoms_cfg = _cfg.get('n_atoms', 'auto')
    if len(n_atoms_set) == 1:
        N_ATOMS = n_atoms_set.pop()
        print(f"N_ATOMS = {N_ATOMS} (uniform)")
    else:
    ...
    for model_name in RUN_MODELS:
        ...
        if HAS_OPTUNA and N_TRIALS > 1:
            ...
            if ck not in graph_cache:
                graph_cache[ck] = build_all_graphs(
                    ...
                    best_hp['max_num_nbr'], n_atoms_pad=N_ATOMS,
                    include_sisj=sisj, shell_edges=_def_shell_edges)
        ...
        if ck not in graph_cache:
            ...
            graph_cache[ck] = build_all_graphs(
                structures, best_hp['cutoff'], best_hp['n_shells'],
                best_hp['max_num_nbr'], n_atoms_pad=N_ATOMS,
                include_sisj=sisj, shell_edges=_se)
        dataset = graph_cache[ck]
