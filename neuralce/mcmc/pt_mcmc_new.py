"""
PT-MCMC v4.1 — Parallel Tempering Monte Carlo with NeuralCE

Changes from v4:
  [1] Exchange inside lax.scan: chunk = n_cycles × (SWAP_INTERVAL steps + exchange)
      → single JIT call, exchange every SWAP_INTERVAL steps (not every chunk)
      → SWAP_INTERVAL is in *steps*, not chunks
  [2] chunk_with_exchange replaces separate replica_chunk_jit + exchange_step

Retained from v4:
  - Vectorized replica exchange (JAX array ops, no Python for-loop)
  - vmap(lax.scan(step)) for replica-internal scan
  - Gumbel trick + lax.switch for branch-free moves
  - Single checkpoint load, vectorized shell one-hot
  - Deduplicated energy computation (single_energy)
...
print(f"{'═' * 70}")
print(f"  PT-MCMC v4.1 — {DATASET_NAME}")
...
def run_pt_mcmc():
    ...
    # ── v4.1: lax.scan over (SWAP_INTERVAL local steps + exchange) cycles
    # One JIT call = CHUNK_SIZE steps total.
    # Inside: n_cycles = CHUNK_SIZE // SWAP_INTERVAL iterations of
    #   [SWAP_INTERVAL local MC steps (vmap + lax.scan)] + [vectorized exchange]
    # Exchange happens every SWAP_INTERVAL steps, all within XLA — no Python loop.

    _swap_interval = max(1, SWAP_INTERVAL)
    n_cycles = CHUNK_SIZE // _swap_interval
    assert n_cycles * _swap_interval == CHUNK_SIZE, \
        f"CHUNK_SIZE ({CHUNK_SIZE}) must be divisible by SWAP_INTERVAL ({_swap_interval})"

    _local_steps_idx = jnp.arange(_swap_interval)

    def _run_one_replica_segment(key, species, spins, E, beta):
        """Run SWAP_INTERVAL local MC steps for a single replica."""
        ...
        (key, species, spins, E, _), _ = lax.scan(
            scan_step, (key, species, spins, E, beta), _local_steps_idx)
        return key, species, spins, E

    _vmap_segment = vmap(_run_one_replica_segment)

    @jax.jit
    def chunk_with_exchange(keys, species_b, spins_b, E_b, betas, ex_key, start_step):
        """One chunk = n_cycles × (SWAP_INTERVAL local steps + exchange).

        Total steps per call = CHUNK_SIZE.
        Exchange every SWAP_INTERVAL steps, all inside XLA.
        """
        def cycle_body(carry, cycle_idx):
            keys, species_b, spins_b, E_b, ex_key = carry

            # (1) SWAP_INTERVAL local MC steps (vmap over replicas)
            keys, species_b, spins_b, E_b = _vmap_segment(
                keys, species_b, spins_b, E_b, betas)

            # (2) Vectorized replica exchange
            step_idx = start_step + (cycle_idx + 1) * _swap_interval
            ex_key, species_b, spins_b, E_b = exchange_fn(
                ex_key, species_b, spins_b, E_b, betas, step_idx)

            return (keys, species_b, spins_b, E_b, ex_key), None

        (keys, species_b, spins_b, E_b, ex_key), _ = lax.scan(
            cycle_body, (keys, species_b, spins_b, E_b, ex_key),
            jnp.arange(n_cycles))

        return keys, species_b, spins_b, E_b, ex_key
    ...
    print(f"    Replica exchange: every {_swap_interval} step(s) "
          f"({n_cycles} exchanges per chunk)")
    ...
    replica_keys, species_batch, spins_batch, current_E, ex_key = \
        chunk_with_exchange(
            replica_keys, species_batch, spins_batch, current_E,
            betas, ex_key, 0)
    ...
    for chunk in range(1, n_chunks):
        replica_keys, species_batch, spins_batch, current_E, ex_key = \
            chunk_with_exchange(
                replica_keys, species_batch, spins_batch, current_E,
                betas, ex_key, global_step)

        global_step += CHUNK_SIZE

        # Save after burn-in (CHUNK_SIZE = save interval)
